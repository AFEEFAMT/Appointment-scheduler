...
class APIRequestError(Exception):
    """Raised when an API request fails application validation."""
...
def _log_pipeline_result(
    ...
    """Log pipeline metadata without storing appointment text."""
    ...
    logger.info(
        ...
        result.trace.extraction.model_used,
    )
...
async def ocr_error_handler(
    ...
    """Handle unreadable or unsupported image inputs."""

    message = str(error)
    lowered_message = message.casefold()

    if "unsupported" in lowered_message:
        status_code = 415
    elif (
        "too large" in lowered_message
        or "maximum" in lowered_message
        or "size limit" in lowered_message
    ):
        status_code = 413
    else:
        status_code = 422
...
async def gemini_configuration_error_handler(
    ...
    return _error_response(
        message=(
            "The AI extraction service is not configured "
            "correctly."
        ),
        status_code=503,
    )
...
async def gemini_quota_error_handler(
    ...
    logger.warning(
        "Gemini quota error: %s",
        error,
    )

    return _error_response(
        message=(
            "The AI extraction service is temporarily "
...
async def gemini_service_error_handler(
    ...
    """Handle failures across all configured Gemini models."""
...
async def extraction_error_handler(
    ...
    """Handle invalid text sent to the extraction service."""
...
async def unexpected_error_handler(
    ...
    """Prevent internal details from reaching API clients."""
...
async def root() -> dict[str, str]:
    """Provide links to API documentation and health status."""
...
async def schedule_from_text(
    ...
    reference_datetime: Annotated[
        ...
        Body(
            description=(
                "Optional ISO-8601 datetime used to "
